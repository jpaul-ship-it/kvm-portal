...
app.get('/api/customers/search', requireCustomerAccess, (req, res) => {
  const q = req.query.q || '';
  const results = all("SELECT id, company_name, customer_type, billing_city, billing_state, is_partner_company FROM customers WHERE company_name LIKE ? AND status='active' ORDER BY company_name LIMIT 20", ['%'+q+'%']);
  res.json(results);
});
...
// Token is set via RESET_TOKEN env variable
app.post('/api/emergency-reset', (req, res) => {
  const { token, new_password } = req.body;
  const expectedToken = process.env.RESET_TOKEN || '';
  
  // Must have a token set in env and it must match
  ...
  console.log('  Emergency admin password reset for user:', admin.username);
...
// One-time role fix — sets all is_admin users to global_admin
app.post('/api/fix-roles', requireAuth, (req, res) => {
  // Only allow if the current user is already is_admin in the DB
  ...
  const result1 = db.run("UPDATE users SET role_type='global_admin' WHERE is_admin=1");
  const result2 = db.run("UPDATE users SET role_type='technician' WHERE is_admin=0 AND (role_type IS NULL OR role_type='' OR role_type='admin')");
  saveDb();
  const admins = all("SELECT username, role_type FROM users WHERE role_type='global_admin'");
  res.json({ ok: true, global_admins: admins });
});
